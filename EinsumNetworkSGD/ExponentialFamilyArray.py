...
class ExponentialFamilyArray(torch.nn.Module):
	...
	def __init__(self, num_var, num_dims, array_shape, num_stats):
		"""
	...
	def forward(self, x):
		...
		# self.suff_stats.requires_grad_(requires_grad=True)
		# self.suff_stats.retain_grad()
		...
		# self.ll.requires_grad_(requires_grad=True)
		# self.ll.retain_grad()
...
class NormalArray(ExponentialFamilyArray):
	...
	def __init__(self, num_var, num_dims, array_shape, min_var=0.0001, max_var=10.):
		super(NormalArray, self).__init__(num_var, num_dims, array_shape, 2 * num_dims)
	...
	def log_normalizer(self, theta):
		log_normalizer = -theta[..., 0:self.num_dims] ** 2 / (4 * theta[..., self.num_dims:]) - 0.5 * torch.log(
			-2. * theta[..., self.num_dims:])
	...
	def _sample(self, num_samples, params, std_correction=1.0):
		with torch.no_grad():
			...
			samples = mu.unsqueeze(0) + std_correction * std.unsqueeze(0) * torch.randn(shape, dtype=mu.dtype,
																						device=mu.device)
...
class BinomialArray(ExponentialFamilyArray):
	...
	def __init__(self, num_var, num_dims, array_shape, N):
		super(BinomialArray, self).__init__(num_var, num_dims, array_shape, num_dims)
...
class CategoricalArray(ExponentialFamilyArray):
	...
	def __init__(self, num_var, num_dims, array_shape, K):
		super(CategoricalArray, self).__init__(num_var, num_dims, array_shape, num_dims * K)
		self.K = K
