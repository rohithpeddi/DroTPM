...
from EinsumNetworkSGD import Graph
from EinsumNetworkSGD.FactorizedLeafLayer import FactorizedLeafLayer, NormalArray
from EinsumNetworkSGD.SumLayer import *
	...
	def __init__(self, graph, args=None):
		...
		for c, layer in enumerate(self.graph_layers[1:]):
			if c % 2 == 0:  # product layer
				einet_layers.append(EinsumLayer(self.graph, layer, einet_layers))
			else:  # sum layer
...
def fetch_likelihoods_for_data(einet, x, labels=None, batch_size=100, marginalize_idx=None):
	with torch.no_grad():
		# Set marginalize indices for allowing estimation of conditional likelihoods
		...
		for batch_count, idx in enumerate(idx_batches):
			...
			outputs = einet(batch_x)
			ll_sample = log_likelihoods(outputs, batch_labels)
			likelihoods.append(ll_sample)

		# Reset marginalize indices before exiting
		if marginalize_idx is not None:
			einet.set_marginalization_idx(None)

		return torch.cat(likelihoods)


def fetch_conditional_likelihoods_for_data(einet, x, batch_size, marginalize_idx):
	joint_lls = fetch_likelihoods_for_data(einet, x, batch_size=batch_size)
	marginal_lls = fetch_likelihoods_for_data(einet, x, batch_size=batch_size, marginalize_idx=marginalize_idx)
	return joint_lls - marginal_lls
