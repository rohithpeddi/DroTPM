...
wandb_run = wandb.init(project="RO-SGD-SPN", entity="utd-ml-pgm")
...
def train_einet_ro(dataset_name, einet, perturbations, train_x, valid_x, test_x, batch_size, learning_rate,
				   weight_decay):
	early_stopping = EarlyStopping(einet, patience=DEFAULT_EINET_PATIENCE,
								   filepath="{}_{}".format(dataset_name, EARLY_STOPPING_FILE),
								   delta=EARLY_STOPPING_DELTA)
	optimizer = optim.Adam(list(einet.parameters()), lr=learning_rate, weight_decay=weight_decay)
	...
	for epoch_count in range(1, MAX_NUM_EPOCHS):
		...
		einet.eval()
		train_dataset = SPN.fetch_adv_data(einet, dataset_name, train_x, test_x, None, perturbations, LOCAL_SEARCH, device,
	...
	for dataset_name in specific_datasets:
		...
		for structure in structures:
			...
			if trained_adv_einet is None:
				...
				trained_adv_einet = train_einet_ro(dataset_name, adv_einet, perturbations, train_x, valid_x,
...
if __name__ == '__main__':
	...
	wandb_run.log({"{}-RO-SGD-LL".format(dataset_name): ll_table})
	for evidence_percentage in EVIDENCE_PERCENTAGES:
		cll_ev_table = cll_tables[evidence_percentage]
		wandb_run.log({"{}-RO-SGD-CLL-{}".format(dataset_name, evidence_percentage): cll_ev_table})
