...
def process(dataset, run, h, N, modeltype, wd):
	savepath = '../checkpoints/%s_run=%u_h=%u_N=%u_%s_wd=%.5f' % (dataset, run, h, N, modeltype, wd)

	try:
		with open('%s.csv' % savepath, 'r+') as csvfile:
			try:
				reader = csv.reader(csvfile)
				fold, train, valid, test, parzen = zip(
					*[[int(row[0]), float(row[1]), float(row[2]), float(row[3]), float(row[4])] for row in reader])
...
if __name__ == "__main__":
	...
	datasets += ["amzn_apparel", "amzn_bath", "amzn_bedding", "amzn_carseats", "amzn_diaper", "amzn_feeding",
				 "amzn_furniture", "amzn_gear", "amzn_gifts", "amzn_health", "amzn_media", "amzn_moms", "amzn_safety",
