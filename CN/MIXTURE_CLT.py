...
class MIXTURE_CLT():
	...
	def EM(self, dataset, max_iter, epsilon):
		...
		for itr in range(max_iter):
			...
			if itr > 0:
				self.mixture_weight = Util.normalize(
					np.einsum('ij->i', clt_weights_list) + 1.0)  # smoothing and Normalize
...
def main_mixture_clt(parms_dict):
	...
	np.savez_compressed(out_file, clt_component=save_list, weights=mix_clt.mixture_weight, valid_ll=valid_ll,
						test_ll=test_ll)
