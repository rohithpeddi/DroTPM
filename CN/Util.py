...
class Util(object):
	...
	def compute_weighted_xycounts(dataset, weights, timing=False):
		...
		prob_xy[:, :, 0, 0] = np.einsum('ij,ik->jk', (dataset == 0).astype(int) * weights[:, np.newaxis],
										(dataset == 0).astype(int))
		prob_xy[:, :, 0, 1] = np.einsum('ij,ik->jk', (dataset == 0).astype(int) * weights[:, np.newaxis],
										(dataset == 1).astype(int))
		prob_xy[:, :, 1, 0] = np.einsum('ij,ik->jk', (dataset == 1).astype(int) * weights[:, np.newaxis],
										(dataset == 0).astype(int))
		prob_xy[:, :, 1, 1] = np.einsum('ij,ik->jk', (dataset == 1).astype(int) * weights[:, np.newaxis],
										(dataset == 1).astype(int))
		if timing:
			print(time.time() - start)
	...
	def compute_edge_potential(xyprob, parents, timing=False):
		...
		# for convenient, the first item is redundant
		edge_potential[0, :, :] = 0
