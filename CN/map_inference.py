...
def compute_xmax_bcnet(bcnet, dataset, x_var, e_var):
	...
	for i in range(len(bcnet_leaf_list)):
	...
	for i in range(dataset.shape[0]):
		...
		for j in range(n_components):
			...
			xmax_prob_temp[j], x_map = cnet.instantiation(datapoint, e_var, evid_flag, bcnet_internal_list[j],
														  bcnet_leaf_list[j])
...
def main_map_cnet(parms_dict):
	...
	# save the max tuple
	if module_type == 'cn':
		new_data_file = output_dir + data_name + '_' + module_type + '_' + str(int(e_percent * 100)) + '_' + str(
			seq) + '.txt'
	else:
		new_data_file = output_dir + data_name + '_' + module_type + '_' + str(int(e_percent * 100)) + '_' + str(
			depth) + '_' + str(seq) + '.txt'
...
def main_map_mt(parms_dict):
	...
	for i in range(mt_module.n_components):
		clt_c = Leaf_tree()
