...
class JunctionTree:
	...
	def add_query_var(self, var_id):
		...
		clique_potential_extend = add_query_var_to_matrix(self.clique_var_arr, self.clique_potential, self.n_varibles,
														  var_id)
...
def get_marginal_JT(jt, evid_list, query_var):
	...
	for i in range(n_varible - 1):
		...
		# swap
		pairwise_marginal_dia[:, 1, 0], pairwise_marginal_dia[:, 0, 1] = pairwise_marginal[:, 0, 1], pairwise_marginal[
																									 :, 1, 0]
...
def main_jt():
	...
	print('clt test set log-likelihood score: ', clt.computeLL(data_test) / data_test.shape[0])
