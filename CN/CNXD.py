...
class CNXD:
	...
	def learnStructureHelper(self, tum, dataset, ids, lamda, beta_function, evid_list, data_ind, next_id=-1,
							 next_weights=np.zeros(2)):
		...
		return [variable, variable_id, p0, p1,
				self.learnStructureHelper(tum, dataset, new_ids, lamda, beta_function, evid_list_0, new_data_ind0),
	...
	def learnStructure(self, tum, dataset, lamda, beta_function):
		...
		# First time learn
		if len(self.tree) == 0:
			self.tree = self.learnStructureHelper(tum, dataset, ids, lamda, beta_function, [],
...
def main_cnxd(parms_dict):
	...
	for i in range(min_depth, max_depth + 1):
		...
		np.savez_compressed(module_dir + data_name + '_' + str(lamda) + '_' + beta_function + '_' + str(i),
							module=main_dict)
