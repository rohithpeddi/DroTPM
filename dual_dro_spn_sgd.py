...
# wandb_run = wandb.init(project="DRO-DUAL-SGD-SPN", entity="utd-ml-pgm")
...
def train_einet_dual_dro(dataset_name, einet, perturbations, train_x, valid_x, test_x, batch_size, learning_rate,
						 weight_decay):
	early_stopping = EarlyStopping(einet, patience=DEFAULT_EINET_PATIENCE,
								   filepath="{}_{}_{}_{}".format(dataset_name, WASSERSTEIN_DUAL, perturbations,
																 EARLY_STOPPING_FILE),
								   delta=EARLY_STOPPING_DELTA)
	optimizer = optim.Adam(list(einet.parameters()), lr=learning_rate, weight_decay=weight_decay)

	attack_update = 1
	num_samples, num_dim = train_x.shape
	lamb_learning_rate = (1/num_samples)
	adv_train_x = train_x
	for epoch_count in range(1, MAX_NUM_EPOCHS):
		train_dataset = TensorDataset(adv_train_x)
		...
		batch_counter = 0
		lamb = float(random.randint(1, 100))
		for inputs in train_dataloader:
			einet.train()
			optimizer.zero_grad()
			...
			if epoch_count > 1:
				lamb_gradient = torch.sum(
					torch.abs(train_x[batch_counter * batch_size:min(num_samples, (batch_counter + 1) * batch_size), :]
							  - adv_train_x[
								batch_counter * batch_size:min(num_samples, (batch_counter + 1) * batch_size),
								:])) - batch_size * perturbations
				objective += lamb * lamb_gradient

				lamb = lamb + lamb_learning_rate * lamb_gradient
		...
		einet.eval()
		if epoch_count % attack_update == 0:
			adv_train_x = local_search_attack.generate_adv_dataset_for_dual(einet, lamb, perturbations, dataset_name, train_x, device, combine=False, batched=True)
	...
	for dataset_name in specific_datasets:
		...
		for structure in structures:
			...
			if trained_adv_einet is None:
				...
				trained_adv_einet = train_einet_dual_dro(dataset_name, adv_einet, perturbations, train_x, valid_x,
...
if __name__ == '__main__':
	...
	for perturbation in DRO_PERTURBATIONS:
		...
		train_meta_dro_spn(run_id=run_id, device=device, specific_datasets=dataset_name,
						   train_attack_type=WASSERSTEIN_DUAL, perturbations=perturbation)

	dataset_wandb_tables = fetch_wandb_table(dataset_name)
	ll_table = dataset_wandb_tables[LOGLIKELIHOOD_TABLE]
	cll_tables = dataset_wandb_tables[CONDITIONAL_LOGLIKELIHOOD_TABLES]

	# wandb_run.log({"{}-DUAL-DRO-LL".format(dataset_name): ll_table})
	# for evidence_percentage in EVIDENCE_PERCENTAGES:
	# 	cll_ev_table = cll_tables[evidence_percentage]
	# 	wandb_run.log({"{}-DUAL-DRO-CLL-{}".format(dataset_name, evidence_percentage): cll_ev_table})
